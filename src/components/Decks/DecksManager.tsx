...
import { Layers, Plus, Trash2, Play, Check, Search, X, Sparkles, BookOpen } from 'lucide-react';
...
export const DecksManager: React.FC<DecksManagerProps> = ({ onStudyDeck }) => {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-16">
      {/* Header */}
      <div className="bg-slate-800/70 border border-slate-700/70 rounded-3xl p-6 sm:p-8 backdrop-blur-md shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
        ...
        {!isCreating && (
          <button
            ...
            className="px-5 py-3 rounded-2xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm flex items-center gap-2 shadow-lg shadow-purple-950/50 transition-all active:scale-95"
          >
      ...
      {/* Creation Modal / Form */}
      {isCreating && (
        <form onSubmit={handleSaveDeck} className="bg-slate-800/90 border-2 border-purple-500/50 rounded-3xl p-6 sm:p-8 backdrop-blur-md shadow-2xl space-y-5 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-xl text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-purple-500"
              />
            </div>
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                Оберіть страви для колоди (Обрано: <span className="text-purple-400 font-black">{selectedDishIds.length}</span>):
              </label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
              <div className="relative sm:col-span-2">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-100 placeholder-slate-500"
                />
              </div>
            ...
            <div className="max-h-60 overflow-y-auto bg-slate-900/80 border border-slate-700/80 rounded-2xl p-2 space-y-1">
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded-xl cursor-pointer flex items-center justify-between text-xs transition-colors ${
                      isChecked 
                        ? 'bg-purple-950/70 border border-purple-600/70 text-purple-200' 
                        : 'hover:bg-slate-800 text-slate-300 border border-transparent'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                        isChecked ? 'bg-purple-600 border-purple-500 text-white' : 'border-slate-600'
                      }`}>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-700">
            <button
              type="button"
              onClick={() => setIsCreating(false)}
              className="px-5 py-2.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-300 font-semibold text-xs transition-colors"
            >
              Скасувати
            </button>
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-bold text-xs shadow-lg shadow-purple-950/50 transition-all active:scale-95"
            >
      ...
      {customDecks.length === 0 ? (
        <div className="bg-slate-800/40 border border-slate-700/60 rounded-3xl p-12 text-center">
          ...
          <button
            type="button"
            onClick={() => setIsCreating(true)}
            className="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold text-xs inline-flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {customDecks.map(deck => (
            <div
              key={deck.id}
              className="bg-slate-800/70 border border-slate-700/80 rounded-2xl p-5 shadow-lg flex flex-col justify-between hover:border-purple-500/50 transition-all duration-200"
            >
              <div>
              ...
              <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-purple-950/40 transition-all active:scale-95"
                >
