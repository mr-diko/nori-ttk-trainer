    ...
    <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-3">
        ...
        >
          ...
          <div>
            <div className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-wider flex items-center gap-1 group-hover:text-emerald-500 transition-colors">
              NORI <span className="text-emerald-700 dark:text-emerald-400 font-extrabold text-xs px-1.5 py-0.5 bg-emerald-100 dark:bg-emerald-950/70 border border-emerald-300 dark:border-emerald-800/60 rounded">ТТК</span>
            </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all border ${
              ignoreDecor
                ? 'bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 border-emerald-400 dark:border-emerald-600 shadow-sm'
                : 'bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-700 hover:text-slate-900 dark:hover:text-slate-200'
            }`}
            title="Перемикач: приховувати декор у картках і тестах"
          >
            <Sparkles className={`w-3.5 h-3.5 ${ignoreDecor ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`} />
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-750 transition-colors"
