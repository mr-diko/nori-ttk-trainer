...
import { 
  ...
  Trash2, Search, Check, AlertCircle, FileSpreadsheet, Moon, Sun
} from 'lucide-react';
...
export const SettingsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-16">
      {/* Header */}
      <div className="bg-slate-800/70 border border-slate-700/70 rounded-3xl p-6 sm:p-8 backdrop-blur-md shadow-xl">
      ...
      <div className="bg-slate-800/60 border border-slate-700/70 rounded-3xl p-6 backdrop-blur-md shadow-xl space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-200 placeholder-slate-500"
              />
            </div>
          </div>

          <div className="max-h-56 overflow-y-auto bg-slate-900/80 border border-slate-750 rounded-2xl p-2 space-y-1">
            {filteredIngredients.map(item => (
              <div
                ...
                className={`p-2 rounded-xl cursor-pointer flex items-center justify-between text-xs transition-colors ${
                  item.isDecor
                    ? 'bg-teal-950/60 border border-teal-700/60 text-teal-200'
                    : 'bg-slate-800/50 hover:bg-slate-800 text-slate-300 border border-transparent'
                }`}
              >
                <div className="flex items-center gap-2">
                  <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center ${
                    item.isDecor ? 'bg-teal-600 border-teal-500 text-white' : 'border-slate-600'
                  }`}>
      ...
      <div className="bg-slate-800/60 border border-slate-700/70 rounded-3xl p-6 backdrop-blur-md shadow-xl space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
          >
        ...
        {importStatus && (
          <div className="p-3 rounded-xl bg-emerald-950/70 border border-emerald-600 text-emerald-300 text-xs flex items-center gap-2">
        ...
        {importError && (
          <div className="p-3 rounded-xl bg-rose-950/70 border border-rose-600 text-rose-300 text-xs flex items-center gap-2">
      ...
      <div className="bg-slate-800/60 border border-slate-700/70 rounded-3xl p-6 backdrop-blur-md shadow-xl space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="px-4 py-2.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold text-xs flex items-center gap-2 cursor-pointer transition-colors">
            <Upload className="w-4 h-4" />
            Відновити з файлу
            <input
              type="file"
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-600/40 font-semibold text-xs flex items-center gap-2 transition-colors ml-auto"
          >
